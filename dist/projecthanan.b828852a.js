...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (menuIcon && navLinks) {
        ...
        document.addEventListener('click', (e)=>{
            if (!e.target.closest('.nav-links') && !e.target.matches('.menu-icon')) navLinks.classList.remove('active');
        });
    ...
    if (currentPage === 'news.html') {
        ...
        if (selectedCountry) // استدعاء وظيفة البحث عن أخبار للدولة المحددة
        setTimeout(()=>{
    ...
    if (currentPage === 'statistics.html') (0, _chartsJs.renderCharts)();
});
if ('serviceWorker' in navigator) window.addEventListener('load', ()=>{
    navigator.serviceWorker.register(require("3e4a993a1cf4f24b")).then((registration)=>{
